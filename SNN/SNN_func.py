...
class Trainer():
    ...
    def train(self, num_epochs, verbosity=1, plot_b=False):
    ...
    def _get_all(self, transform: Callable = lambda *args, **kwargs: args[0]):
        ...
        return all_targets.numpy(), all_predictions.numpy(), all_accuracy.numpy()
        ...
        for i in range(n_tasks):
            ...
            if plot_type == "2D":
                ...
                if "E" in title[i] or "sigma" in title[i]:
                    r_min = min(targets[:, select[i]].min(), predictions[:, select[i]].min()).item()
                    r_max = max(targets[:, select[i]].max(), predictions[:, select[i]].max()).item()
                    r = [r_min, r_max]                    
                else:
                    r = [0, 9]
